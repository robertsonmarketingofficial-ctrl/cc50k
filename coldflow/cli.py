...
from . import campaigns, db, inboxes, leads, planner, replies, report, scheduler, sender
...
def cmd_plan(args, settings, conn):
    p = planner.PlanInputs(
        ...
        inboxes_per_domain=args.per_domain, sequence_steps=args.steps, inbox_cost_month=args.inbox_cost,
...
def cmd_inboxes(args, settings, conn):
    if args.action == "import":
        ...
        for r in conn.execute("SELECT email, password_env FROM inboxes WHERE status<>'retired' ORDER BY email"):
            print(f"export {r['password_env']}=''   # app password for {r['email']}")
    else:
...
def cmd_schedule(args, settings, conn):
    _print_counter(f"Queue for {_date(args.date)}", scheduler.build_queue(conn, _date(args.date), settings))
...
def cmd_replies(args, settings, conn):
    _print_counter("Inbox sweep", replies.poll(conn, days=args.days))
    cmd_health(args, settings, conn)
...
def cmd_daily(args, settings, conn):
    """What cron runs each sending morning: sweep replies, build the queue, send, refresh the dashboard."""
    cmd_replies(args, settings, conn)
    cmd_schedule(args, settings, conn)
    cmd_send(args, settings, conn)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--inbox-cost", type=float, default=7.20)
    p.add_argument("--lead-cost", type=float, default=0.02)
